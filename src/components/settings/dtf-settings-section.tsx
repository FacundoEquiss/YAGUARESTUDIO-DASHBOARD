...
export function DtfSettingsSection() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({ title: "Configuración guardada", description: "Los valores se aplicarán a las nuevas cotizaciones." });
    } catch {
      toast({ title: "Error", description: "No se pudo guardar. Probá de nuevo.", variant: "destructive" });
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-lg">
      <Card className="border-none shadow-md bg-card">
        <CardContent className="p-6 space-y-6">
          ...
          <div className="space-y-2">
            <Label htmlFor="price" className="text-sm font-bold">Precio por Metro Lineal</Label>
            <div className="relative">
              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground font-bold text-lg">$</span>
              <Input id="price" type="number" value={price} onChange={(e) => setPrice(e.target.value)} className="pl-9 font-bold text-lg h-14" />
            </div>
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <Input id="width" type="number" value={width} onChange={(e) => setWidth(e.target.value)} className="pr-12 font-bold text-lg h-14" />
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input id="baseMargin" type="number" value={baseMargin} onChange={(e) => setBaseMargin(e.target.value)} className="pl-8 font-bold h-12" />
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground font-bold">$</span>
                <Input id="wholesaleMargin" type="number" value={wholesaleMargin} onChange={(e) => setWholesaleMargin(e.target.value)} className="pl-8 font-bold h-12" />
              </div>
        ...
        <CardContent className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input id="pressPassThreshold" type="number" min="1" value={pressPassThreshold} onChange={(e) => setPressPassThreshold(e.target.value)} className="font-bold h-12" />
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground font-bold">$</span>
                <Input id="pressPassExtraCost" type="number" min="0" value={pressPassExtraCost} onChange={(e) => setPressPassExtraCost(e.target.value)} className="pl-8 font-bold h-12" />
              </div>
      ...
      <Card className="border-none shadow-md bg-card">
        <CardContent className="p-6 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground font-bold">$</span>
              <Input id="talleSurcharge" type="number" min="0" value={talleSurcharge} onChange={(e) => setTalleSurcharge(e.target.value)} className="pl-8 font-bold h-12" />
            </div>
      ...
      <Button size="lg" className="w-full rounded-2xl" onClick={handleSave} disabled={saving}>
        <Save className="w-5 h-5 mr-2" />
        {saving ? "Guardando…" : "Guardar ajustes del cotizador"}
      </Button>
    </div>
  );
}
